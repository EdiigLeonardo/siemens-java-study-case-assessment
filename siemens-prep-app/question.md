...
- [ ] Java: enum usado de forma frágil (ordinal)
- [ ] Java: `equals()`/`hashCode()` incompleto
- [ ] Spring: injeção por campo em vez de construtor
- [ ] Spring: `@Transactional` sem efeito (self-invocation)
- [ ] Spring: exception handler genérico a mais (mascara 400 como 500)
- [ ] Hexagonal: adapter de entrada a falar direto com adapter de saída
- [ ] Hexagonal: entidade JPA exposta na API
- [ ] REST: verbo HTTP errado numa alteração de estado
- [ ] REST/DTO: sem validação (`@NotBlank`, etc.)
- [ ] Query: N+1 numa listagem
- [ ] SQL: FK sem índice
- [ ] Segurança: CORS demasiado aberto (`*` + credentials)
- [ ] Segurança: sem controlo de role nos endpoints
- [ ] Angular: subscribe sem `ngOnDestroy` (memory leak)
- [ ] Angular: subscribe sem tratamento de erro
- [ ] Angular: `*ngFor` sem `trackBy`
